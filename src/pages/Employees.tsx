          ...
          {employees.length === 0 ? (
            ...
            employees.map((emp) => (
              <tr key={emp.id} className="border-b border-border hover:bg-accent/50 transition-colors">
                ...
                <td className="px-4 py-3 text-xs">{emp.project}</td>
...
export default function Employees() {
  ...
  const dailyEmployees = employees.filter((e) => e.type === "Daily");
  ...
  return (
    <Layout>
      <PageHeader
        title="Employees"
        subtitle="Fixed salary & daily wage workers — All Projects"
