...
import {
  ...
  type CreateNonConsumableLedgerInput,
  type UpdateNonConsumableLedgerInput,
} from "../services/nonConsumableLedgerService.js";
...
export async function create(req: AuthRequest, res: Response) {
  try {
    ...
    const { itemId } = req.params;
    const entry = await createNonConsumableLedgerEntry(
      { userId: actor.userId, email: actor.email, role: actor.role },
      { ...req.body, itemId } as CreateNonConsumableLedgerInput
    );
...
export async function update(req: AuthRequest, res: Response) {
  try {
    const actor = req.user!;
    const entry = await updateNonConsumableLedgerEntry(
      { userId: actor.userId, email: actor.email, role: actor.role },
      req.params.id,
      req.body as UpdateNonConsumableLedgerInput
    );
    res.json(entry);
  } catch (err) {
...
export async function remove(req: AuthRequest, res: Response) {
  try {
    const actor = req.user!;
    await deleteNonConsumableLedgerEntry(
      { userId: actor.userId, email: actor.email, role: actor.role },
      req.params.id
    );
    res.status(204).send();
    ...
    const status =
      msg.includes("not found") ? 404 : msg.includes("Cannot delete") ? 400 : 500;
    res.status(status).json({ error: msg });
  }
}
